...
import { teamLogos } from '../data/driverMeta';
...
const StandingsPage: React.FC = () => {
  ...
  const teamStandings = Object.values(
    drivers.reduce((acc, d) => {
      if (!acc[d.teamZh]) {
        acc[d.teamZh] = { team: d.teamZh, teamId: d.teamId, points: 0, color: d.teamColor, drivers: [] as string[] };
      }
      acc[d.teamZh].points += d.points;
      acc[d.teamZh].drivers.push(d.nameZh);
      return acc;
    }, {} as Record<string, { team: string; teamId: string; points: number; color: string; drivers: string[] }>)
                ...
                <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-800 flex-shrink-0">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      if (step === '0' && driver.fallbackImage) {
                        img.dataset.fallbackStep = '1';
                        img.src = driver.fallbackImage;
                      } else if (step !== '2') {
                        img.dataset.fallbackStep = '2';
                        img.src = `https://placehold.co/120x120/1e1e2e/ffffff?text=%23${driver.number}`;
                      }
                    }}
                  />
                </div>
          ...
          {teamStandings.map((team, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="w-10 h-10 rounded-lg overflow-hidden bg-white/10 flex items-center justify-center flex-shrink-0 p-1">
                  <img
                    src={teamLogos[team.teamId] || ''}
                    alt={team.team}
